...
public class CPU {
	...
	public void dump() {
		...
		System.out.println("TC: " + registers.get("TC"));
		System.out.println();
	}
	 ...
	 */
	public void fetch(InstructionMemory iMemory, Memory dMemory, Cache cache) {
		dMemspeed = dMemory.speed;
		decode(iMemory.imem[Integer.parseInt(registers.get("PC").substring(2),16)], cache);
	}
	
	public int decode(String command, Cache cache) {
		...
		//lw takes greater of mem speed or 1 cycle
			...
			if(cache.enabled) {
				if(registers.get(tgt).equals("0xff")) {
					wait = 1;
				}
				else if(cache.readHit(registers.get(tgt))) {
					wait = 1;
				}
				else {
					wait=5;
		...
		//sw takes the greater of the memory speed or 1 cycle
			...
			if(cache.enabled) {
				...
				else if(cache.writeHit(registers.get(tgt))) {
					wait = 1;
				}
				else {
					for(int i=0; i<8; i++) {
						if(cache.cflags[i].equals("W")) {
							wait = 5;
						}
						else {
							wait = 1;
	...
	// executing method for when cache is disabled
	public boolean execute(Memory dMem, InstructionMemory iMem) {
		...
		case "101":
			...
			int mval = Integer.parseInt(dMem.mem[Integer.parseInt(registers.get(tgt).substring(2),16)].substring(2),16);
			if(mval<16) {
				setreg(dest, "0x0" + dMem.mem[Integer.parseInt(registers.get(tgt).substring(2),16)].substring(2));
			}
			else {
	...
	// returns true if an instruction was executed, false if not
	// execution method that is used whenever cache is enabled
	public boolean cachedexecute(Memory dMem, InstructionMemory iMem, Cache cache) {
		switch(ins) {
		...
		case "101":
			...
			int readval = cache.cread(registers.get(tgt).substring(2),dMem);
			//non-zero means cache hit
			if(readval != 0) {
				if(readval>16) {
					registers.put(dest, "0x" + Integer.toHexString(readval).toUpperCase());
				}
				else {
					registers.put(dest, "0x0" + Integer.toHexString(readval).toUpperCase());
				}
			}
		...
		//sw commmand
		case "110":
			cache.cstore(registers.get(tgt).substring(2).toUpperCase(),registers.get(src).substring(2).toUpperCase(),dMem);
		...
		//halt command
		case "111":
			//halts execution of processor increments PC
			wait = -1;
			incPC();
			registers.put("TC",String.valueOf(Integer.parseInt(registers.get("TC"))-1));
		default:
			return false;
		}
	}
