			...
			if(device.equals("clock")) {
				...
				case "tick": 
					...
					for(int i=0; i<tickCount; i++) {
						if(simCache.enabled || simCPU.wait==0) {
							if(simIODev.schedule.size()>0 && simCPU.registers.get("TC").equals(simIODev.schedule.peek()[0])) {
								//tick has a scheduled IO event
								if(IObusy <= 0) {
									simIODev.executeCommand(simDataMemory);
									if(!simCache.enabled) {
										simCPU.wait = 5;
									}
									IObusy = 5;
				...
				case "dump":
					simCache.dump();
					break;
				default: System.out.println("INVALID CACHE COMMAND");
					break;
				}				
			}
			
			else if(device.equals("iodev")) {
				switch (command) {
				case "reset":
					simIODev.reset();
					break;
				case "load":
					iodata = new File(reader.next());
					simIODev.load(iodata);
					break;
				default: System.out.println("INVALID IODEV COMMAND");
					break;					
				}
			}
		}
